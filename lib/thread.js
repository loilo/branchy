...
process.on('message', ({ callback, args, env }) => {
  ...
  const globals = Object.entries(Object.assign({ require: sourceRequire }, env))
  const globalsCode = globals
    .map(([ constName, value ], index) => `let ${constName} = arguments[0][${index}]`)
    .join('; ')
  ...
  const fn = new Function(globalsCode + '; return Reflect.apply(\n' + callback + '\n, null, Array.from(arguments).slice(1))')
